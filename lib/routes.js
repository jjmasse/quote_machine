...
Router.map(function() {
	...
	data: function() { // create the documents and create assign the relationship
			DocumentData.insert(GlobalValues.blankDocument, function(error, id) {
				...
				var newRowSet = [GlobalHelpers.generateUniqueRowData()];
				Session.set('document', id);
				RowData.insert({rows: newRowSet}, function(error, id) {
					Session.set('rows', id); // assign the generated rows information in session
